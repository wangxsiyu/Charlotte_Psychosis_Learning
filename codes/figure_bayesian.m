...
clear stat;
for di = 1:nd
     files = dir(fullfile(dirs(di).folder, dirs(di).name, '*stat.mat'));
     for fi = 1:length(files)
         stat(di, fi) = importdata(fullfile(files(fi).folder, files(fi).name)).stats;
     end
end
%%
for di = 1:5
...
plot(dic(:,:)');
xlim([0.5 5.5]);
set(gca, 'xtick', 1:5, 'xticklabel', {files.name}, 'XTickLabelRotation', 45);
legend({dirs.name})
ylabel('DIC');
%%
for di = 1:nd
